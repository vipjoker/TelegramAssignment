...
public class StackedBarChart implements Renderer {
    ...
    public StackedBarChart(DrawData drawData, ChartData chartData) {
...
//        isTouched = false;
    ...
    private void init() {
        ...
        graphPaintY0.setStyle(Paint.Style.FILL);
//        graphPaintY0.setStrokeWidth(0.5f);
    ...
    public void update(long elapsedTimeMs) {
        chartData.updateTotal();
        final long maxX = chartData.getMaxX(0);
        final long minX = chartData.getMinX(0);
        final long maxY = chartData.getMaxTotal(drawData.getMinScale(), drawData.getMaxScale());
        drawData.setMaxX(maxX);
        drawData.setMinX(minX);
        drawData.setMaxTotal(maxY);
    ...
    public void draw(Canvas canvas, long time) {
        ...
        drawGrid( drawData.getMaxTotal(), canvas);
    ...
    private void drawChartY0(Canvas canvas, long minX, long maxX, long maxY, LineData[] lineDates) {
        if (chartData != null) {
            ...
            float startPointX = 0;
            float startPointY = 0;
            long maxTotal = drawData.getMaxTotal();
            ...
            for (int i = 0; i < chartData.size(); i++) {
                ...
                final long total = chartData.getTotal(i);
                float graphX = getScaledX(x, minX, maxX);
                ...
                for (int j = 0; j < lineDates.length; j++) {
                    ...
                    final float frac = MathUtil.map(total, 0, maxTotal, 0, 1);
                    float graphY = MathUtil.map(y, 0, total, 0, 1) * frac;

                    graphX = Math.round(graphX);


                    float height = this.height - bottomOffset;
                    barRect.set(startPointX, height - (startPointY * height ) , graphX, height - (graphY + startPointY) * height );
                    if(drawData.isShowInfo()){
                        if(lastX > barRect.left && lastX < barRect.right){
                            canvas.drawRect(barRect, graphPaintY0);
                        }else{
                            graphPaintY0.setAlpha(125);
                            canvas.drawRect(barRect, graphPaintY0);
    ...
    private void drawCursor(Canvas canvas, float lastX, long minX, long maxX) {
        ...
        final String value = String.valueOf(chartData.getTotal(maxIndex));
