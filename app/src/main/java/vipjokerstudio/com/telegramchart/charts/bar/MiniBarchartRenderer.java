...
public class MiniBarchartRenderer implements Renderer {
    ...
    public MiniBarchartRenderer(DrawData scaleData, ChartData chartData) {
    ...
    private void drawChartY0(Canvas canvas, long minX , long maxX, long maxY, LineData lineData) {
        if (chartData != null) {
            ...
            for (int i = 0; i < chartData.size(); i++) {
                ...
                if (i != 0) {
                    barRect.set(startPointX, height, graphX, height - graphY);
                    if(barRect.right > leftRect.right && barRect.left < rightRect.left){
                        graphPaint.setColor(selectedColor);
                    }else{
